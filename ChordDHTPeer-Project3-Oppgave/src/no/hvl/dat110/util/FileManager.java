...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		// hash the replica
		BigInteger hashed =  Hash.hashOf(replicaFileName);
		// store the hash in the replicafiles array.
		replicafiles[i] = hashed;
		}
    ...
    public int distributeReplicastoPeers() throws RemoteException {
    	...
    	for( int i = 0; i< replicafiles.length ; i++) {
    		BigInteger filID = (BigInteger)replicafiles[i];
    		NodeInterface filesucc = chordnode.findSuccessor(filID);
    		if (filesucc != null) {
    			filesucc.addKey(filID);
    			
    			if(index == i) {
    				filesucc.saveFileContent(filename, filID, bytesOfFile, true);
    			}else {
    				filesucc.saveFileContent(filename, filID, bytesOfFile, false);
    			}
    		}
    		counter++;
    	}
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for(int i = 0; i < replicafiles.length; i++) {
			...
			if(filesucc != null) {
				Map<BigInteger, Message> successormap= filesucc.getFilesMetadata();
				succinfo.add(successormap.get(filID));
			}
		}
