...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		if(stub!= null) {
		BigInteger successorID = stub.getNodeID();
		BigInteger nodeID = this.node.getNodeID();
		
		Boolean cond = Util.computeLogic(key, nodeID.add(new BigInteger ("1")), successorID);
		//Boolean cond = Util.computeLogic(nodeID.add(new BigInteger ("1")),key,  successorID);
	...
	// if logic returns false; call findHighestPredecessor(key)
		...
		// do return highest_pred.findSuccessor(key) - This is a recursive call until logic returns true
			NodeInterface highest_pred = findHighestPredecessor(key); //= findHighestPredecessor(key); node.getPredecessor()
	...
	/*
	 * public NodeInterface getSuccessor() { // TODO Auto-generated method stub
	 * return successor; }
	 * 
	 * 
	 * public void setSuccessor(NodeInterface successor) { this.successor =
	 * successor; }
	 * 
	 */
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		List<NodeInterface> fingerEntries = this.node.getFingerTable();
		
		// starting from the last entry, iterate over the finger table
		
		// for each finger, obtain a stub from the registry
		int size = fingerEntries.size()-1;
		
		for(int i = 0; i < fingerEntries.size()-1; i++) {
			int a = size-i;
			
			
			NodeInterface fsuccessor = fingerEntries.get(a);
			try {
				BigInteger fsuccessorID = fsuccessor.getNodeID();
				NodeInterface fsuccessorNode = Util.getProcessStub(fsuccessor.getNodeName(), fsuccessor.getPort());
			
				boolean cl =
						Util.computeLogic(
								fsuccessorID, this.node.getNodeID().add(new BigInteger("1")),
								key.subtract(new BigInteger("1")));
				if(cl) {
					return fsuccessorNode;
				}
				...
				// TODO: handle exception
					...
					/*
					Util.getProcessStub(fingerEntries.get(i).getNodeName(),
										fingerEntries.get(i).getPort()); */
