...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		// if yes, acquireLock
		Boolean all = areAllMessagesReturned(messages.size());
		if (all) {
			acquireLock();
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		case 2: {
			...
			BigInteger messageID = message.getNodeID();
			...
			/*
			 * if (clocker == ownClocker) { if (messageID.compareTo(nodeID) < -1) {
			 * 
			 * //message.setAcknowledged(true); NodeInterface mstub =
			 * Util.getProcessStub(message.getNodeIP(), message.getPort());
			 * mstub.onMutexAcknowledgementReceived(message); } else { queue.add(message); }
			 * } if (clocker < ownClocker) {
			 * 
			 * message.setAcknowledged(true); NodeInterface mstub =
			 * Util.getProcessStub(message.getNodeIP(), message.getPort());
			 * mstub.onMutexAcknowledgementReceived(message);
			 * 
			 * } else {
			 * 
			 * queue.add(message); }
	...
	private boolean areAllMessagesReturned(int numvoters) throws RemoteException {
		...
		int ackSize = queueack.size();

		// clear the queueack
		
		// return true if yes and false if no
		if (ackSize == numvoters) {
			queueack.clear();
			return true;
